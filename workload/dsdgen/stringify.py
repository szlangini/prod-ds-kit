...
from workload.dsdgen.config import key_skew_rules, mcv_skew_rules, null_skew_rules, stringify_rules
...
QUERY_FILTER_COLUMNS_PATH = REPO_ROOT / "config" / "query_filter_columns.txt"


def _read_column_list(path: Path) -> frozenset:
    try:
...
def _query_referenced_columns() -> frozenset:
    ...
    """
    return _read_column_list(QUERY_REFERENCED_COLUMNS_PATH)


@lru_cache(maxsize=1)
def _query_filter_columns() -> frozenset:
    """Column names compared against a literal anywhere in the workload -- the
    subset of query-referenced columns whose concentration can actually empty
    results. Generated by tools/analyze_query_column_roles.py. Used when the
    MCV profile sets query_exclusion_scope: filtered (columns referenced only
    as join predicates / group keys / order keys / aggregate inputs remain
    eligible: concentrating them shrinks groups but cannot empty a query)."""
    return _read_column_list(QUERY_FILTER_COLUMNS_PATH)
...
# Join-key skew never redirects EVERY occurrence of a key: f1 is capped so each
# key value keeps at least this share of its natural occurrences (in expectation),
# which is what the no-empty argument in docs/key-skew.md relies on.
KEY_SKEW_MIN_RETENTION = 0.05

# Small-dimension NULL floor. A NULL probability is a per-column statistical target
# taken from the fleet distribution and is independent of the table's size. On a
# dimension with only a handful of rows (store = 12 and warehouse = 5 at SF1,
# warehouse = 15 at SF100) a probability of 0.9 leaves 0-1 non-NULL rows, so every
# predicate that touches the column (`w_state in (...)`, `s_market_desc is not null`)
# empties by construction. The floor guarantees that at least NULL_MIN_NONNULL_ROWS
# naturally non-NULL rows of every nulled column stay non-NULL. Where it binds, the
# rule's probability is lowered to an exact per-table threshold: the per-row decision
# stays `hash < probability` (identical in Python and C++), but the threshold sits
# between the m-th and (m+1)-th smallest row hash so that exactly m = n - k rows are
# nulled instead of a Bernoulli draw whose variance on 12 rows exceeds the floor
# itself. Large tables are never touched: the cap can only bind for n < k / (1 - p),
# i.e. below ~800 rows at the profile's maximum probability of 0.995.
NULL_MIN_NONNULL_ROWS = 4
# 1-row dsdgen metadata table: no query touches it, un-nulling it would only shift
# the NULL-share CDF that the paper reports.
NULL_FLOOR_EXEMPT_TABLES = frozenset({"dbgen_version"})
# Files above this size belong to tables far beyond the binding range; skip counting.
NULL_FLOOR_MAX_FILE_BYTES = 64 * 1024 * 1024
# The same floor is ONE shared invariant across both injection axes: no axis
# ever rewrites more than (1 - retention) of a column's non-null cells, so the
# natural value tail always survives. Without it, a target above the non-null
# fraction saturates at f1 = 1.0 and the column collapses onto its natural
# dominant value (count(DISTINCT) = 1) -- on aggregate-input measures whose
# dominant value is 0.00 that zeroes every SUM/AVG, and relative comparisons
# ("total > 1.2 * avg(state)", the q81 family) become unsatisfiable for every
# parameter choice. The fleet reference itself measures dominance < 1.0 (the
# top bucket ends at 0.99), so the floor is the model boundary, not a tuning
# knob.
MCV_MIN_RETENTION = KEY_SKEW_MIN_RETENTION
...
def process_tbl(
    ...
    key_injector: "KeySkewInjector | None" = None,
    ...
    key_rules_present = bool(key_injector and table_name in key_injector.rules)
    rewrite_required = (
        bool(table_rules) or null_rules_present or mcv_rules_present or key_rules_present
    )
    ...
    with in_path.open("r", encoding="utf-8", errors="surrogateescape") as src, out_path.open(
        ...
        for raw_line in src:
            ...
            # Key skew must run before stringification: decisions and replacement
            # values operate on raw dsdgen keys/identities, and the hot key then
            # gets the same STR formatting as every other cell in its domain.
            if key_injector:
                key_injector.apply_to_row(table_name, row)
...
def _stable_unit_hash(seed: int, *parts: object) -> float:
    ...
    for part in parts:
        hasher.update(b"\x1f")
        # surrogateescape restores the original bytes of non-UTF-8 input read
        # with errors="surrogateescape", matching the C++ raw-byte hash.
        hasher.update(str(part).encode("utf-8", errors="surrogateescape"))
...
class NullColumnRule:
    ...
    probability: float
    name: str
    # Set only where the small-dimension floor binds: the profile's original
    # probability and the number of naturally non-NULL rows the floor was
    # computed over (see NULL_MIN_NONNULL_ROWS).
...
class NullInjector:
    def __init__(
        ...
        *,
        ...
        raw_floor = cfg.get("min_nonnull_rows", NULL_MIN_NONNULL_ROWS)
        try:
            self.min_nonnull_rows: int = max(0, int(raw_floor))
        except (TypeError, ValueError):
            self.min_nonnull_rows = NULL_MIN_NONNULL_ROWS
        raw_exempt = cfg.get("floor_exempt_tables")
        self.floor_exempt_tables: Set[str] = (
            {str(t).lower() for t in raw_exempt if t}
            if isinstance(raw_exempt, (list, tuple, set, frozenset))
            else set(NULL_FLOOR_EXEMPT_TABLES)
        )
        self.floor_applied: List[Dict[str, Any]] = []
        ...
        if self.enabled and self.rules and source_data_dir is not None:
            self._apply_small_table_floor(Path(source_data_dir))

    def _apply_small_table_floor(self, data_dir: Path) -> None:
        """Enforce NULL_MIN_NONNULL_ROWS on small tables (see the constant's comment).

        Only tables whose data files are present in ``data_dir`` and small enough
        to be row-counted are considered; the floor can never bind on large tables.
        Rules the floor does not bind on are left untouched (Bernoulli draw as
        before), so data for every other column is byte-identical.
        """
        k = self.min_nonnull_rows
        if k <= 0:
            return
        files_by_table: Dict[str, List[Path]] = {}
        for extension in DATA_EXTENSIONS:
            for data_file in sorted(data_dir.glob(f"*{extension}")):
                table_name = _table_name_from_filename(data_file.name)
                if table_name and table_name in self.rules:
                    files_by_table.setdefault(table_name, []).append(data_file)

        for table_name, table_rules in list(self.rules.items()):
            if table_name.lower() in self.floor_exempt_tables:
                continue
            files = files_by_table.get(table_name)
            if not files:
                continue
            try:
                ...
                continue
            if total_bytes > NULL_FLOOR_MAX_FILE_BYTES:
                continue
            # (partition token, row index, fields) exactly as apply_to_row sees them:
            # the token is the file name and the index counts every line of the file.
            rows: List[Tuple[str, int, List[str] | None]] = []
            for data_file in files:
                with data_file.open("r", encoding="utf-8", errors="surrogateescape") as src:
                    for idx, raw_line in enumerate(src):
                        stripped = raw_line.rstrip("\n")
                        rows.append((data_file.name, idx, stripped.split("|") if stripped else None))

            new_rules: List[NullColumnRule] = []
            for rule in table_rules:
                hashes: List[float] = []
                for token, idx, fields in rows:
                    if fields is None or rule.index >= len(fields):
                        continue
                    cell = fields[rule.index]
                    if cell == "" or cell == self.null_marker or cell == "\\N":
                        continue  # naturally NULL: never counts as a survivor
                    hashes.append(_stable_unit_hash(self.seed, table_name, rule.name, token, idx))
                n = len(hashes)
                cap = 1.0 - (float(k) / float(n)) if n > 0 else 0.0
                if rule.probability <= cap:
                    new_rules.append(rule)
                    continue
                m = max(0, n - k)  # rows to null: exactly n - k survivors remain
                hashes.sort()
                threshold = 0.0 if m <= 0 else (hashes[m - 1] + hashes[m]) / 2.0
                self.floor_applied.append(
                    {
                        "table": table_name,
                        "column": rule.name,
                        "rows": n,
                        "target_probability": rule.probability,
                        "nulled_rows": m,
                        "probability": threshold,
                    }
                )
                if threshold <= 0.0:
                    continue  # nothing may be nulled on this column
                new_rules.append(
                    NullColumnRule(
                        index=rule.index,
                        probability=threshold,
            ...
            if new_rules:
                self.rules[table_name] = new_rules
            else:
                del self.rules[table_name]
    ...
    def apply_to_row(self, table_name: str, row: List[str], row_index: int, partition: str | None = None) -> None:
        ...
        for rule in table_rules:
            ...
            # Mirror the C++ backend: a cell that is already NULL (dsdgen-native
            # empty cell or marker) is left as is, so both backends emit identical
            # bytes for any null marker and the floor's survivor count is exact.
            if current == "" or current == self.null_marker or current == "\\N":
                continue
    ...
    def _apply_explicit_rules(self, schema: SchemaInfo) -> None:
        for qualified, probability in self.explicit_probabilities.items():
            ...
            if not columns:
                continue
            if _is_protected_key_column(meta, column_name):
                print(
                    f"[null-skew] ignoring explicit probability for key/NOT NULL "
                    f"column {qualified} (nulling keys or identities would break joins)."
                )
                continue
...
class MCVInjector:
    def __init__(
        ...
        *,
        ...
        # Never concentrate a column whose skew could empty query results. Two
        # scopes: "referenced" (default, conservative) excludes every column the
        # workload references anywhere; "filtered" excludes only columns compared
        # against literals (config/query_filter_columns.txt) -- columns used
        # purely as join predicates, GROUP BY / ORDER BY keys, or aggregate
        # inputs stay eligible, since concentrating them shrinks groups or
        # changes aggregate values but cannot collapse a result to empty.
        self.query_exclusion_scope: str = str(
            cfg.get("query_exclusion_scope", "referenced")
        ).lower()
        if self.query_exclusion_scope == "filtered":
            self.exclude_columns |= _query_filter_columns()
        else:
    ...
    def _resolve_target(
        ...
        *,
        ...
        if target_non_null >= 1.0:
            f1 = 1.0  # saturation: clamped to the retention floor below
        elif share >= 1.0:
            f1 = 0.0
        else:
            f1 = (target_non_null - share) / (1.0 - share)
        # Retention floor: never rewrite every non-null cell -- the natural value
        # tail must survive (see MCV_MIN_RETENTION above).
        return max(0.0, min(1.0 - MCV_MIN_RETENTION, f1)), value
    ...
    def _apply_explicit_top5_rules(self, schema: SchemaInfo) -> None:
        for qualified, cfg in self.explicit_top5_rules.items():
            ...
            if _is_protected_key_column(meta, column_name):
                print(
                    f"[mcv-skew] ignoring explicit top5 rule for key/NOT NULL "
                    f"column {qualified} (concentrating keys or identities would break joins)."
                )
                continue
            index = next((i for i, col in enumerate(columns) if col.lower() == column_name), None)
            if index is None:
                continue
            canonical_name = columns[index]
            f1 = min(share, share / max(1, len(values)), 1.0 - MCV_MIN_RETENTION)
...
class KeySkewColumnRule:
    ...
    f1: float
    value: str
    # "ticket": decide per sale identity (ticket/order number); "entity": decide
    # per key value, so an entity keeps its whole history or none of it.
    granularity: str = "ticket"


# Join-key skew targets: fact-side FK columns, grouped by sales channel. A
# ticket-granular redirect decision hashes (channel, domain, identity), where
# the identity column (ticket/order number) is shared between a channel's sales
# and returns files -- both files independently reach the same decision and
# replacement value, so the sales<->returns joins on (item_sk, ticket/order_number)
# and the sr_* = ss_* column invariants stay coherent without cross-file state.
# Entity-granular domains (profile `entity_domains`, default: customer) hash the
# key VALUE instead (no channel, no ticket): a customer either keeps its complete
# purchase history in every channel or is absorbed by the hot key everywhere.
# Covers every fact-table FK domain that is safe to redirect (41 channel/domain
# pairs, 75 columns). Deliberately absent:
#   - `item`: every sales/returns primary key is (item_sk, ticket/order_number),
#     so redirecting item FKs creates duplicate composite PKs engines reject.
#   - `inventory`: its primary key is (date_sk, item_sk, warehouse_sk) -- all
#     FKs -- so any redirect risks duplicate PKs.
# return_date/return_time get their own hot values (independent of the sold
# date): redirected returns can therefore predate their sale; no query treats
# that as a correctness constraint, but it is a documented plausibility caveat.
def _channel(channel, identity_columns, domains):
    return {
        "channel": channel,
        "identity_columns": identity_columns,
        "domains": {
            name: {"canonical": cols[0], "mirrors": tuple(cols[1:])}
            for name, cols in domains.items()
        },
    }


KEY_SKEW_CHANNELS: Sequence[Mapping[str, Any]] = (
    _channel(
        "store",
        {"store_sales": "ss_ticket_number", "store_returns": "sr_ticket_number"},
        {
            "date": [("store_sales", "ss_sold_date_sk")],
            "time": [("store_sales", "ss_sold_time_sk")],
            "customer": [
            ...
            "promo": [("store_sales", "ss_promo_sk")],
            "return_date": [("store_returns", "sr_returned_date_sk")],
            "return_time": [("store_returns", "sr_return_time_sk")],
            "reason": [("store_returns", "sr_reason_sk")],
        },
    ),
    _channel(
        "catalog",
        ...
        {
            "date": [("catalog_sales", "cs_sold_date_sk")],
            "ship_date": [("catalog_sales", "cs_ship_date_sk")],
            "time": [("catalog_sales", "cs_sold_time_sk")],
            "customer": [
            ...
            "promo": [("catalog_sales", "cs_promo_sk")],
            "return_date": [("catalog_returns", "cr_returned_date_sk")],
            "return_time": [("catalog_returns", "cr_returned_time_sk")],
            "reason": [("catalog_returns", "cr_reason_sk")],
        },
    ),
    _channel(
        "web",
        ...
        {
            "date": [("web_sales", "ws_sold_date_sk")],
            "ship_date": [("web_sales", "ws_ship_date_sk")],
            "time": [("web_sales", "ws_sold_time_sk")],
            "customer": [
            ...
            "web_site": [("web_sales", "ws_web_site_sk")],
            "ship_mode": [("web_sales", "ws_ship_mode_sk")],
            "warehouse": [("web_sales", "ws_warehouse_sk")],
            "promo": [("web_sales", "ws_promo_sk")],
            "return_date": [("web_returns", "wr_returned_date_sk")],
            "return_time": [("web_returns", "wr_returned_time_sk")],
            "reason": [("web_returns", "wr_reason_sk")],
        },
    ),
)


class KeySkewInjector:
    """OPT-IN join-key skew: redirect a share of fact-table FKs to one hot key.

    Runs BEFORE stringification so decisions and replacement values operate on
    raw dsdgen values; the hot key then passes through the same STR formatting
    as every other cell, keeping stringified join key spaces consistent.

    Per (channel, domain) pair a target top-1 share T is drawn from the
    fleet-calibrated max_value_buckets, and f1 is solved with the same
    null-compensated monotone formula as the MCV profile (g = natural non-null
    fraction; keys receive no injected NULLs). The hot key is the canonical
    column's natural dominant value, so referential integrity holds and every
    key value keeps >= (1 - f1) of its original occurrences -- FK->PK joins can
    shrink but never empty.

    Granularity: most domains decide per sale identity (ticket/order), thinning
    every key uniformly. Domains listed in the profile's `entity_domains`
    (default: customer) decide per key VALUE instead -- a share of customers is
    absorbed by the hot key in every channel while the rest keep their complete
    histories, so year-over-year and cross-channel customer templates keep their
    populations (per-ticket thinning shrank SF10 three-channel repeat customers
    from 1,968 to 6).
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.scale_factor: int | None = (
            int(cfg["scale_factor"]) if cfg.get("scale_factor") is not None else None
        )
        self._buckets = self._normalize_buckets(cfg.get("max_value_buckets") or [])
        # Blacklist of "channel.domain" pairs that stay unskewed (risk control for
        # domains whose conjunction with others empties queries).
        self.exclude_pairs: Set[str] = {
            str(p).lower() for p in cfg.get("exclude_pairs", []) if p
        }
        # Per-domain target caps (domain name -> max T). Date-family domains are
        # the most query-coupled (nearly every template filters a date window and
        # the sales->return->re-buy chains hop several date FKs), so their
        # targets get a harder ceiling than the global bucket table.
        self.domain_caps: Dict[str, float] = {
            str(k).lower(): float(v)
            for k, v in (cfg.get("domain_caps") or {}).items()
        }
        # Domains whose redirect decision is taken per KEY VALUE (entity) rather
        # than per sale identity (see class docstring).
        self.entity_domains: Set[str] = {
            str(d).lower() for d in (cfg.get("entity_domains") or []) if d
        }
        # An entity is ONE dimension row in every channel: the hot key of an
        # entity domain is shared across channels (the anchor channel's natural
        # dominant value; the largest channel, store, by default). Per-channel hot
        # keys would give the whale customer store rows under one key and
        # catalog/web rows under another, emptying every cross-channel customer
        # join (q4/q11/q25/q74 ...) of the whale's 60-76% of the traffic.
        self.entity_anchor_channel: str = str(cfg.get("entity_anchor_channel") or "store").lower()
        self.entity_hot_values: Dict[str, Dict[str, str]] = {}
        self.rules: Dict[str, List[KeySkewColumnRule]] = {}
        self.pair_targets: Dict[str, Dict[str, Any]] = {}
        if not (self.enabled and self._buckets):
            return

        canonical_map: Dict[str, List[str]] = {}
        for channel_cfg in KEY_SKEW_CHANNELS:
            for domain_cfg in channel_cfg["domains"].values():
                table_name, column_name = domain_cfg["canonical"]
                canonical_map.setdefault(table_name, []).append(column_name)
        if natural_stats is not None:
            stats = {str(k).lower(): v for k, v in natural_stats.items()}
        else:
            raw_cache_dir = cfg.get("ndv_cache_dir") or os.getenv("PRODDS_NDV_CACHE_DIR")
            cache_dir = (
                Path(str(raw_cache_dir)).expanduser() if raw_cache_dir else DEFAULT_NDV_CACHE_DIR
            )
            stats = _load_natural_mcv_stats(
                schema,
                canonical_map,
                source_data_dir=source_data_dir,
                cache_dir=cache_dir,
        ...
        self.rules = self._build_rules(schema, stats)
    ...
    def apply_to_row(self, table_name: str, row: List[str]) -> None:
        if not self.enabled:
            return
        table_rules = self.rules.get(table_name)
        if not table_rules:
        ...
        for rule in table_rules:
            ...
            if rule.granularity == "entity":
                # Entity granularity: hash the key VALUE (no channel, no ticket)
                # so an entity is redirected in every row of every channel below
                # its channel's f1, or keeps its complete history (nested by f1
                # across channels).
                h = _stable_unit_hash(self.seed, "key-skew-entity", rule.domain, current)
            else:
                if rule.identity_index >= len(row):
                    continue
                identity = row[rule.identity_index]
                if identity in ("", self.null_marker, "\\N"):
                    continue
                # Decision depends ONLY on (channel, domain, identity) -- no table,
                # column, partition, or row index -- so mirror columns in sales and
                # returns files reach the identical decision independently.
                h = _stable_unit_hash(self.seed, "key-skew", rule.channel, rule.domain, identity)
            if h < rule.f1:
                row[rule.index] = rule.value

    def pairs_summary(self) -> Dict[str, Dict[str, Any]]:
        """Per-(channel, domain) hot key and rates, for the data manifest (and
        for query-side parameter curation that wants to target the hot keys)."""
        return {key: dict(entry) for key, entry in self.pair_targets.items()}
    ...
    def _derive_target(self, channel: str, domain: str) -> float:
        pick = _stable_unit_hash(self.seed, "key-skew", channel, domain, "bucket")
        cumulative = 0.0
        ...
        for bucket in self._buckets:
            ...
            if pick <= cumulative:
                chosen = bucket
                break
        span = max(0.0, chosen["max"] - chosen["min"])
        sample = _stable_unit_hash(self.seed, "key-skew", channel, domain, "target")
        return max(0.0, min(1.0, chosen["min"] + sample * span))

    def _build_rules(
        self, schema: SchemaInfo, stats: Mapping[str, "NaturalColumnStat"]
    ) -> Dict[str, List[KeySkewColumnRule]]:
        rules: Dict[str, List[KeySkewColumnRule]] = {}
        # Shared hot key per entity domain: the anchor channel's canonical natural
        # dominant value, else the first channel (in KEY_SKEW_CHANNELS order) that
        # has a natural stat for the domain.
        entity_values: Dict[str, Dict[str, str]] = {}
        for channel_cfg in KEY_SKEW_CHANNELS:
            channel = str(channel_cfg["channel"]).lower()
            for domain, domain_cfg in channel_cfg["domains"].items():
                if domain.lower() not in self.entity_domains:
                    continue
                canonical_table, canonical_column = domain_cfg["canonical"]
                stat = stats.get(f"{canonical_table.lower()}.{canonical_column.lower()}")
                if stat is None or not stat.value:
                    continue
                current = entity_values.get(domain.lower())
                if current is None or (
                    channel == self.entity_anchor_channel
                    and current["channel"] != self.entity_anchor_channel
                ):
                    entity_values[domain.lower()] = {"channel": channel, "value": stat.value}
        self.entity_hot_values = entity_values
        for channel_cfg in KEY_SKEW_CHANNELS:
            channel = str(channel_cfg["channel"])
            identity_columns: Mapping[str, str] = channel_cfg["identity_columns"]
            for domain, domain_cfg in channel_cfg["domains"].items():
                if f"{channel}.{domain}".lower() in self.exclude_pairs:
                    continue
                canonical_table, canonical_column = domain_cfg["canonical"]
                stat = stats.get(f"{canonical_table.lower()}.{canonical_column.lower()}")
                # Without a natural stat there is no known-valid key to amplify;
                # never synthesize key values (referential integrity).
                if stat is None or not stat.value:
                    continue
                target = self._derive_target(channel, domain)
                cap = self.domain_caps.get(domain.lower())
                if cap is not None:
                    target = min(target, cap)
                if target <= 0.0:
                    continue
                # Same solve as MCVInjector._resolve_target with f_null = 0:
                # keys receive no injected NULLs, so g is the natural non-null
                # fraction. f1 <= 0 -> already at/above target, leave untouched.
                g = 1.0 - stat.null_rate
                if g <= 0.0:
                    continue
                target_non_null = target / g
                share = max(0.0, min(1.0, stat.share_nn))
                if target_non_null >= 1.0:
                    f1 = 1.0
                elif share >= 1.0:
                    ...
                    f1 = (target_non_null - share) / (1.0 - share)
                # Retention floor: a target above g (saturation) must not collapse
                # the column onto the hot key -- keep (1 - f1) >= KEY_SKEW_MIN_RETENTION.
                f1 = max(0.0, min(1.0 - KEY_SKEW_MIN_RETENTION, f1))
                if f1 <= 0.0:
                    continue

                granularity = "entity" if domain.lower() in self.entity_domains else "ticket"
                shared = entity_values.get(domain.lower()) if granularity == "entity" else None
                hot_value = shared["value"] if shared else stat.value
                columns = [(canonical_table, canonical_column)]
                columns.extend(domain_cfg["mirrors"])
                applied: List[str] = []
                for table_name, column_name in columns:
                    ...
                    if identity_column is None:
                        continue
                    index = _column_index(schema, table_name, column_name)
                    identity_index = _column_index(schema, table_name, identity_column)
                    if index is None or identity_index is None:
                        continue
                ...
                if applied:
                    self.pair_targets[f"{channel}.{domain}"] = {
                        "canonical": f"{canonical_table}.{canonical_column}",
                        "value": hot_value,
                        "entity_anchor": shared["channel"] if shared else None,
                        "target": target,
                        "f1": f1,
                        "granularity": granularity,
                        "columns": applied,
        ...
        for table_rules in rules.values():
            table_rules.sort(key=lambda rule: (rule.index, rule.name))
        return rules


def _is_protected_key_column(meta: Mapping[str, Any], column_name: str) -> bool:
    """Key-like or NOT NULL columns (join keys, identities) that explicit
    NULL/MCV rules must never touch -- they bypass the normal eligibility."""
    lowered = column_name.lower()
    key_like = {c.get("name", "").lower() for c in meta.get("key_like_columns") or []}
    key_like |= {c.get("name", "").lower() for c in meta.get("varchar_keys") or []}
    not_nulls = {c.get("name", "").lower() for c in meta.get("not_null_columns") or []}
    return lowered in key_like or lowered in not_nulls
...
def _run_cpp_rewrite(
    ...
    *,
    ...
    enable_key_skew: bool | None,
    key_skew_seed: int | None,
    key_skew_profile: str | None,
    key_skew_overrides: Mapping[str, Any] | None,
    backend_mode: str,
) -> tuple[int, int] | None:
    ...
    with tempfile.TemporaryDirectory(prefix="stringify_cpp_") as tmpdir:
        ...
        export_rewrite_rules(
            ...
            enable_key_skew=enable_key_skew,
            key_skew_seed=key_skew_seed,
            key_skew_profile=key_skew_profile,
            key_skew_overrides=key_skew_overrides,
            source_data_dir=output_path,
        )
        try:
            rules_payload = yaml.safe_load(rules_path.read_text(encoding="utf-8")) or {}
        except (OSError, yaml.YAMLError):
            rules_payload = {}
        key_rules_requested = bool(
            isinstance(rules_payload, dict)
            and (rules_payload.get("key_skew") or {}).get("enabled")
        )
        if key_rules_requested:
            # Capability probe on an EMPTY directory before touching real data: a
            # stale binary would silently drop the key-skew rules while rewriting
            # in place, after which a python fallback could no longer run safely.
            probe_dir = tmp / "capability_probe"
            probe_dir.mkdir()
            probe_summary = tmp / "probe_summary.json"
            ...
            if probe.returncode == 0 and probe_summary.exists():
                try:
                    probe_payload = json.loads(probe_summary.read_text(encoding="utf-8"))
                except (OSError, json.JSONDecodeError):
                    probe_payload = {}
            if not probe_payload.get("key_skew_applied"):
                if backend_mode == "cpp":
                    raise RuntimeError(
                        "C++ stringify backend does not support the requested key-skew "
                        "rules (stale binary?). Rebuild via `make stringify_cpp` in "
                        "workload/dsdgen/."
                    )
                print(
                    "[key-skew] cpp binary lacks key-skew support -- "
                    "falling back to python backend."
                )
                return None
            entity_requested = any(
                str(rule.get("granularity", "ticket")) == "entity"
                for table_rules in (
                    (rules_payload.get("key_skew") or {}).get("rules") or {}
                ).values()
                for rule in (table_rules or [])
                if isinstance(rule, dict)
            )
            if entity_requested and not probe_payload.get("key_skew_entity_supported"):
                if backend_mode == "cpp":
                    raise RuntimeError(
                        "C++ stringify backend predates entity-granular key skew "
                        "(stale binary?). Rebuild via `make stringify_cpp` in "
                        "workload/dsdgen/."
                    )
                print(
                    "[key-skew] cpp binary lacks entity-granular key skew -- "
                    "falling back to python backend."
                )
                return None
        args = [
        ...
        # From here on the binary may already have rewritten data files in place;
        # a python fallback would apply the pipeline a second time on top of
        # them, so every failure is fatal regardless of backend mode.
        if result.returncode != 0:
            raise RuntimeError(
                f"C++ stringify backend failed with exit code {result.returncode} after "
                "starting the in-place rewrite; not falling back to python (data files "
                "may be partially rewritten -- regenerate the dsdgen output before retrying)."
            )
...
def _rewrite_single(
    ...
    key_injector: "KeySkewInjector | None",
...
def rewrite_tbl_directory(
    ...
    enable_key_skew: bool | None = None,
    key_skew_seed: int | None = None,
    key_skew_profile: str | None = None,
    key_skew_overrides: Mapping[str, Any] | None = None,
) -> Tuple[int, int]:
    """
    ...
    Args:
        ...
        enable_key_skew: Override for the OPT-IN join-key skew toggle (default off).
        key_skew_seed: Optional deterministic seed override for key skew decisions.
        key_skew_profile: Optional profile name from config/key_skew_profiles.yml.
        key_skew_overrides: Additional override mapping merged into the key skew configuration.
    ...
    key_cfg = key_skew_rules(overrides=key_skew_overrides, profile=key_skew_profile)
    if enable_key_skew is not None:
        key_cfg["enabled"] = enable_key_skew
    elif key_skew_profile is not None:
        # Requesting a profile explicitly opts the axis in.
        key_cfg["enabled"] = True
    if key_skew_seed is not None:
        key_cfg["seed"] = key_skew_seed
    if ndv_cache_dir is not None:
        key_cfg["ndv_cache_dir"] = str(ndv_cache_dir)
    if scale_factor is not None:
        key_cfg["scale_factor"] = int(scale_factor)
    key_injector = KeySkewInjector(
        schema,
        key_cfg,
        null_marker=null_cfg.get("null_marker", ""),
        source_data_dir=output_path,
    )
    has_key_rules = key_injector.enabled and key_injector.has_rules
    ...
    if key_injector.enabled:
        pair_bits = ", ".join(
            f"{pair}: {entry['value']}@f1={entry['f1']:.3f}"
            for pair, entry in sorted(key_injector.pair_targets.items())
        )
        print(f"[key-skew] pairs={len(key_injector.pair_targets)} ({pair_bits})")
    ...
    for data_file in data_files:
        ...
        table_has_key_skew = has_key_rules and table_name in key_injector.rules
        if (
            not table_has_stringify
            and not table_has_nulls
            and not table_has_mcv
            and not table_has_key_skew
        ):
            continue
    ...
    if task_count == 0:
        ...
        stringification_cfg.write_json(
            manifest_path,
            {
                ...
                "null_small_table_floor": {
                    "min_nonnull_rows": null_injector.min_nonnull_rows,
                    "exempt_tables": sorted(null_injector.floor_exempt_tables),
                    "capped": list(null_injector.floor_applied),
                },
                ...
                "key_skew_enabled": bool(has_key_rules),
                "key_skew_profile": key_cfg.get("profile"),
                "key_skew_tier_alias": key_cfg.get("tier_alias"),
                "key_skew_seed": key_cfg.get("seed"),
                "key_skew_pairs": key_injector.pairs_summary(),
    ...
    if backend_mode in {"auto", "cpp"}:
        cpp_result = _run_cpp_rewrite(
            ...
            mcv_profile=mcv_profile,
            mcv_overrides=mcv_overrides,
            enable_key_skew=key_cfg.get("enabled"),
            key_skew_seed=key_skew_seed,
            key_skew_profile=key_skew_profile,
            key_skew_overrides=key_skew_overrides,
        ...
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            futures = [
                executor.submit(
                    ...
                    key_injector if has_key_rules else None,
                    stat_keys,
                )
...
def build_rewrite_rules(
    *,
    ...
    key_cfg = key_skew_rules(overrides=key_skew_overrides, profile=key_skew_profile)
    if enable_key_skew is not None:
        key_cfg["enabled"] = bool(enable_key_skew)
    elif key_skew_profile is not None:
        key_cfg["enabled"] = True
    if key_skew_seed is not None:
        key_cfg["seed"] = int(key_skew_seed)
    if ndv_cache_dir is not None:
        key_cfg["ndv_cache_dir"] = str(ndv_cache_dir)
    if scale_factor is not None:
        key_cfg["scale_factor"] = int(scale_factor)
    ...
    def _serialize_null_rules() -> Dict[str, List[Dict[str, Any]]]:
        ...
        for table_name, table_rules in (null_injector.rules or {}).items():
            ...
            for rule in table_rules:
                ...
                if rule.target_probability is not None:
                    # Small-dimension floor bound: keep the profile's target visible.
                    entry["target_probability"] = rule.target_probability
                    entry["floor_rows"] = rule.floor_rows
                entries.append(entry)
            serialized[table_name] = entries
        return serialized
    ...
    def _serialize_key_skew_rules() -> Dict[str, List[Dict[str, Any]]]:
        serialized: Dict[str, List[Dict[str, Any]]] = {}
        for table_name, table_rules in (key_injector.rules or {}).items():
            serialized[table_name] = [
                {
                    "index": rule.index,
                    "name": rule.name,
                    "identity_index": rule.identity_index,
    ...
    return {
        ...
        "key_skew": {
            "enabled": bool(key_injector.enabled and key_injector.has_rules),
            "seed": int(key_cfg.get("seed", 0)),
            "profile": key_cfg.get("profile"),
            "pairs": key_injector.pairs_summary(),
            "rules": _serialize_key_skew_rules(),
        },
    }
