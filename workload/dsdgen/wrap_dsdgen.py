...
def _run_dsdgen_parallel(binary: Path, args: Sequence[str], num_children: int) -> int:
    ...
    # dsdgen -PARALLEL writes one chunk per child as <table>_<child>_<N>.dat. Concatenate
    # each table's chunks (in child order) into <table>.dat and delete the chunks, so the
    # downstream rewrite + loaders (which map a filename stem -> table) see a single file
    # per table, exactly as in the single-threaded path. The union of the chunks is the
    # full table (dsdgen partitions rows across children; no duplication).
    out_dir: Optional[Path] = None
    for i, tok in enumerate(args):
        up = tok.upper()
        if up in ("-DIR", "--DIR") and i + 1 < len(args):
            out_dir = Path(args[i + 1]).expanduser().resolve()
        elif up.startswith(("-DIR=", "--DIR=")):
            out_dir = Path(tok.split("=", 1)[1]).expanduser().resolve()
    if out_dir is not None:
        chunk_re = re.compile(rf"^(?P<table>.+)_(?P<child>\d+)_{num_children}\.dat$")
        groups: dict = {}
        for f in out_dir.glob(f"*_*_{num_children}.dat"):
            m = chunk_re.match(f.name)
            if m:
                groups.setdefault(m.group("table"), []).append((int(m.group("child")), f))
        for table, chunks in groups.items():
            chunks.sort(key=lambda t: t[0])
            target = out_dir / f"{table}.dat"
            with target.open("wb") as outf:
                for _, cf in chunks:
                    with cf.open("rb") as inp:
                        shutil.copyfileobj(inp, outf)
            for _, cf in chunks:
                cf.unlink()
        print(f"[parallel] Merged chunks for {len(groups)} tables -> <table>.dat in {out_dir}")
