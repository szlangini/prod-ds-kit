...
Prod-DS ships three NULL profile configurations. They share the same exclusion
set and differ in column selection fraction `f` and bucket weights.

The **medium** tier is derived from production fleet telemetry and serves as the
default. The **high** tier reuses the medium tier's six bucket boundaries and
shifts weight toward higher null rates. The **low** tier concentrates mass at
lower null rates and tightens its upper three boundaries accordingly — `[0.10,
0.25)`, `[0.25, 0.50)`, `[0.50, 0.80)` in place of the `[0.10, 0.40)`,
`[0.40, 0.80)`, `[0.80, 0.995)` shown in the table below.
