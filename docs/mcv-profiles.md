...
3. **Not LITERAL-FILTERED by the query workload** (`query_exclusion_scope:
   filtered`, the shipped default since the 2026-09 recalibration). Only columns
   the workload compares against literals are excluded
   (`config/query_filter_columns.txt`, generated by
   `tools/analyze_query_column_roles.py`) — concentrating those can collapse
   predicate selectivity and empty results. Columns referenced purely as join
   predicates, GROUP BY / ORDER BY keys, or aggregate inputs stay eligible:
   concentrating them shrinks groups or shifts aggregate values but cannot empty
   a result. The legacy conservative scope (`referenced`: exclude every column
   named anywhere in the workload, `config/query_referenced_columns.txt`) remains
   available as a config override. Join keys are covered by the separate key-skew
   axis (`docs/key-skew.md`): FK→PK redirection cannot empty results, so it may
   target the fact-table FK columns this profile still excludes as key-like.
...
- **Retention floor:** `f1` is additionally clamped to `1 − 0.05`, so at least
  5 % of non-null cells always keep their natural values. A fully collapsed
  column (`count(DISTINCT) = 1`) would make aggregate-comparison queries
  unsatisfiable (e.g. `total > 1.2 × avg(state)` when a measure's dominant
  value is `0.00`), and the fleet reference itself measures dominance < 1.0.
  `tools/validate_distributions.py` guards this invariant after every
  generation.
...
calibrated default (recalibrated 2026-09 together with the key-skew axis: the key
tier is capped at 0.80, so the MCV bucket table is top-heavy and carries the
fleet's extreme [0.90, 0.99) mass on non-conjoining columns).

Target top-1 share buckets `B_T` (weights):

| Range `T` [lo, hi) | Low  | Med (default) | High |
|--------------------|-----:|--------------:|-----:|
| [0.01, 0.10)       | 0.30 | 0.04          | 0.01 |
| [0.10, 0.30)       | 0.30 | 0.05          | 0.02 |
| [0.30, 0.50)       | 0.25 | 0.07          | 0.05 |
| [0.50, 0.70)       | 0.12 | 0.09          | 0.07 |
| [0.70, 0.90)       | 0.03 | 0.15          | 0.15 |
| [0.90, 0.99)       | --   | 0.60          | 0.70 |

| Parameter           | Low  | Med (default) | High |
|---------------------|-----:|--------------:|-----:|
| Column fraction `f` | 0.45 | 0.85          | 1.0  |

Only the **medium/default** tier is calibrated to the production fleet (jointly
with `key_fleet_default`); `mcv_low` and `mcv_high` are softer/harder variants
sharing the same role-scoped eligibility.

## Calibration target (SF10, STR=5 default, NULL+MCV+KEY at medium)

Share of columns with `max_value_share ≥` threshold. With the role-scoped
eligibility and the key-skew axis, the realized curve now tracks the production
fleet across the whole range (mean |gap| ≈ 4 pp; scale-stable SF1→SF10):

| max-value ≥ | TPC-DS base | Prod-DS pre-K (paper v1) | Prod-DS (default, measured) | Redshift fleet |
|-------------|------------:|-------------------------:|----------------------------:|---------------:|
| ≥0.01       | 60.1        | 59.2                     | 79.5                        | 73             |
| ≥0.10       | 36.1        | 40.3                     | 66.2                        | 60             |
| ≥0.30       | 18.6        | 26.6                     | 50.1                        | 49             |
| ≥0.50       | 13.8        | 22.4                     | 45.0                        | 41             |
| ≥0.70       | 11.4        | 14.7                     | 31.9                        | 33             |
| ≥0.90       | 8.6         | 9.8                      | 19.4                        | 25             |

Measured with WorkloadLens on SF10 STR=5 data (`max_count/row_count`, NULLs in
the denominator). The remaining ≥0.90 shortfall is saturation-limited (columns
with injected NULLs cannot reach a 0.9 total-row share). Query-safety is now
enforced by construction (literal-filter exclusion + non-emptiable FK
redirection) and verified by the 107-query gate: 0 errors; the handful of
queries whose parameter draws newly intersect the hot keys are healed via
documented per-query seed overrides (`configs/seed_overrides_*.yml`).
