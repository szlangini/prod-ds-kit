...
def _run_join_query(
    *,
    ...
    if combined.exists():
        ...
        # shutil.move (not Path.replace/os.replace) so this works when the temp
        # dir and the output dir are on different filesystems — e.g. the output
        # under a Docker volume mount, where an atomic rename raises EXDEV.
        shutil.move(str(combined), str(target))
