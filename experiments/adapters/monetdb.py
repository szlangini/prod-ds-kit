...
class MonetDBAdapter(EngineAdapter):
    ...
    def supports_planning(self) -> bool:
        # MonetDB exposes a plan via `EXPLAIN <sql>` (see build_sql), so the harness CAN time a
        # plan-only stage just like DuckDB/CedarDB. Previously False -> wall_time_ms_planning was
        # always None (missing MonetDB planning line in fig11). Enabled for plan/exec split.
        return True
