...
class DuckDBAdapter(EngineAdapter):
    ...
    def build_sql(self, sql: str, stage: str) -> str:
        statements = [
            f"PRAGMA threads={self.global_cfg.threads}",
            # E3 UNION ALL fan-in reaches 2048 inputs; the default binder depth limit of 1000
            # rejects >~256-way unions ("Max expression depth limit of 1000 exceeded"). Raise it so
            # the full fan-in ladder (U512/U1024/U2048) binds. Harmless for normal-depth queries.
    ...
    def build_command_file(self, sql_file: str, stage: str) -> List[str]:
        if self.execution_mode == "native":
            cli = self.native_cfg.get("cli_path", "duckdb")
            db_path = self.database_path or ":memory:"
            return ["bash", "-c", f"{cli} {db_path} < {sql_file}"]
