...
Engine installation and Python/runtime setup are handled by the top-level
`reproduce.sh --init` (pinned engine versions — see the repo-root
`REPRODUCIBILITY.md`) or, hermetically, by the repo-root `Dockerfile`.
The per-engine database loaders the harness invokes live under
`experiments/scripts/load_<engine>.sh`.
