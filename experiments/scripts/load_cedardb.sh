...
# Refresh statistics BEFORE timed execution — paper protocol (§6.3: each engine runs its ANALYZE).
echo "[load] ANALYZE (refresh statistics, paper protocol)..."
"${PSQL_BASE[@]}" -d "${CEDAR_DB}" -c "ANALYZE;" || echo "[load] ANALYZE unavailable; skipped"
