...
# Scaling-ladder level parsed from a query_id, e.g. "query_join_J64" -> 64, "query_union_U200"
# -> 200. Used for ladder early-abandon when join_count/union_count aren't set (workload_compare
# file path). Anchored on the join_J / union_U prefix so plain workload ids (query_19) don't match.
_LADDER_RE = re.compile(r"(?:join_J|union_U)(\d+)", re.IGNORECASE)
...
class BenchmarkRunner:
    ...
    def run(self) -> None:
        ...
        for system_name in systems:
            ...
            try:
                ...
                elif self.experiment == "union_scaling":
                    records = self._run_union_scaling(adapter, writer, system_dir)
    ...
    def _run_union_scaling(self, adapter, writer: ResultWriter, system_dir: Path) -> List[Dict[str, Any]]:
        cfg = self.cfg.experiments.get("union_scaling")
        if not cfg:
            raise RuntimeError("experiments.union_scaling missing in config.")

        union_counts_raw = cfg.get("union_counts") or [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048]
        union_counts = [int(x) for x in union_counts_raw]

        output_dir = cfg.get("output_dir") or (self.run_root / "generated" / "union_scaling")
        output_dir = Path(output_dir)
        ensure_dir(output_dir)

        queries: List[QuerySpec] = []
        for union_count in union_counts:
            query_id = f"union_U{union_count}"
            sql, err = self._generate_union_query(cfg, union_count, output_dir)
            queries.append(
                QuerySpec(
                    query_id=query_id,
                    sql=sql,
        ...
        validation = self._validate_queries(adapter, queries, system_dir)
        self._warmup(adapter, queries, "union_scaling")
        _, records = self._run_queries(adapter, queries, writer, 0, validation_map=validation)
        return records

    def _generate_union_query(self, cfg: Dict[str, Any], union_count: int, out_dir: Path) -> tuple[Optional[str], Optional[str]]:
        command_tmpl = cfg.get("generator_command")
        if not command_tmpl:
            return None, "union_scaling.generator_command not configured"

        output_file = out_dir / f"union_U{union_count}.sql"
        ...
        if not result.stdout.strip():
            return None, "Union generator produced empty SQL"
    ...
    def _run_queries(
        ...
        *,
        ...
        # Ladder early-abandon: join/union scaling levels are monotone in cost — once a level
        # times out (or errors), every higher level will too, so skip same-or-higher levels
        # instead of driving each into the wall. Confirmed intent: "once J64 times out,
        # J128/J256/J512… don't need to run." The level comes from join_count/union_count, or
        # (for the workload_compare file path E2/E3 use) is parsed from the query_id …J<n>/…U<n>.
        # Gate on a HOMOGENEOUS ladder: every query is a J*/U* rung. E1's prodds workload is
        # heterogeneous (query_1…99 plus a few J*/U*), so it must NOT trigger the cutoff there.
        def _ladder_level(q) -> Optional[int]:
            if q.join_count is not None:
                return int(q.join_count)
            if q.union_count is not None:
                return int(q.union_count)
            m = _LADDER_RE.search(q.query_id or "")
            return int(m.group(1)) if m else None
        _real_queries = [q for q in queries if q.sql]
        ladder_abandon = early_abandon and bool(_real_queries) and all(
            _ladder_level(q) is not None for q in _real_queries
        )
        ladder_cutoff: Optional[int] = None

        for query in queries:
            level = _ladder_level(query)
            if ladder_abandon and ladder_cutoff is not None and level is not None and level >= ladder_cutoff:
                run_index += 1
                record = {
                    ...
                    "status": "skipped_ladder_abandon",
                    "error_type": "skipped",
                    "error_message": f"ladder abandon: a level >= {ladder_cutoff} already timed out/failed",
                    "return_code": None,
                ...
                self.logger.info("%s: skipped (ladder abandon — a level >= %s already failed).", query.query_id, ladder_cutoff)
                continue
            ...
            for rep in range(1, repetitions + 1):
                ...
                if early_abandon and result.status != "success":
                    ...
                    # Monotone scaling ladder: this level failed, so same-and-higher levels
                    # are skipped (enforced at the top of the query loop).
                    if ladder_abandon and level is not None:
                        ladder_cutoff = level if ladder_cutoff is None else min(ladder_cutoff, level)
                    break
