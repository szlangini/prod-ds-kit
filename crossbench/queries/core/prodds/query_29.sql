...
,count(*) as item_count
 ...
 ,min((select min(d_date) from date_dim where d_year = 2001 and d_moy = 10)) as min_sales_date
 ,max(cast((select max(d_date) from date_dim where d_year = 2001 and d_moy = 10) as timestamp)) as max_sales_ts
 from  (select * from ss 
        union all
        select * from cs 
        union all
